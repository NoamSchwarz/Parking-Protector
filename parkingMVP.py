...
#TODO if I change the file to notebook_sequential_images then get_num_of_images cant fint the path for some reason
IMG_FILE_PATH = os.path.abspath("notebook_sequential_images_V2")
IMAGE_PATH_TEMPLATE = IMG_FILE_PATH + "\image_{}.jpg"
ALL_SPOTS_EMPTRY_IMG = IMG_FILE_PATH + r"\all_spots_empty.jpg"
...
def mark_parking_spot():
    base_image = read_image(ALL_SPOTS_EMPTRY_IMG)
...
def main():
    ...
    first_image = read_image(IMAGE_PATH_TEMPLATE.format(1))
    first_image_crop = crop_image(first_image, parking_mark)

    #TODO: replace get_num_of _images with function of read_next_image which will not be dependant on the source of the images
    ...
    for image_index in range(2, num_of_images):
        ...
        second_image = read_image(IMAGE_PATH_TEMPLATE.format(image_index))
